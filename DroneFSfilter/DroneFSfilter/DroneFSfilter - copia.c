...
--*/
{
	...
	PDRONEFS_STREAM_HANDLE_CONTEXT dronefsContext;
	...
	if (FltObjects->FileObject != NULL) {
		...
		if (NT_SUCCESS(status)) {
			...
			const wchar_t *cmp = L"lenaprotected";
			if (wcsstr(fileName.Buffer, cmp)) {

				DbgPrint("[PREREAD] Encontrado el fichero: %wZ", &fileName);
				...
				try {
					...
					DbgPrint("[PREREAD] Trying to allocate context stream handle");
					...
					if (NT_SUCCESS(status)) {
						...
						dronefsContext->RescanRequired = TRUE;

						DbgPrint("[PREREAD] Trying to set context stream");
	...
	if (!NT_SUCCESS(status)) {
		...
		if (origBuf == NULL) {
			...
			if (DroneFSData.ClientPort != NULL) {
				...
				DbgPrint("[DroneFSFilter]Getting streamhandle");
				...
				if (NT_SUCCESS(status)) {
					...
					if (notification == NULL) {
						...
						while (filePointer < fileSize) {
							...
							DbgPrint("filepointer=%Iu", filePointer);
							...
							DbgPrint("filesize=%Iu", fileSize);
							DbgPrint("bufferSize=%lu", bufferSize);
							...
							try {
								...
								if (STATUS_SUCCESS == status) {
									...
									safe = ((PDRONEFS_REPLY)notification)->SafeToOpen;
			...
			/*try {
				...
				RtlCopyMemory(arrayRead,
					origBuf,
					1);
				DbgPrint("[POSTREAD]ArrayRead viejo: %s", &arrayRead[0]);

				//cifrar
				if (arrayRead[0] == 0x41) {
					arrayRead[0] = 0x47;

					DbgPrint("[POSTREAD]ArrayRead nuevo: %s", &arrayRead[0]);
					RtlCopyMemory(origBuf,
						arrayRead,
						1);
				}
				...
				DbgPrint("[POSTREAD]Error al copiar");
			}*/
		}
	}
	...
	if (FltObjects->FileObject != NULL) {
		...
		if (NT_SUCCESS(status)) {
			...
			if (wcsstr(fileName.Buffer, cmp)) {
				...
				DbgPrint("[PREWRITE]Archivo: %wZ", &nameInfo->Name);
				...
				try {
					...
					try {
						...
						/*try {
							UCHAR arrayWrite[1];
							RtlCopyMemory(arrayWrite,
								origBuf,
								1);
							DbgPrint("[PREWRITE]arrayWrite viejo: %s", &arrayWrite[0]);

							//cifrar
							if (arrayWrite[0] == 0x47) {
								arrayWrite[0] = 0x41;

								DbgPrint("[PREWRITE]arrayWrite nuevo: %s", &arrayWrite[0]);
								RtlCopyMemory(newBuf,
									arrayWrite,
									1);
							}
							...
							DbgPrint("[PREWRITE]Error al copiar");
						}*/
